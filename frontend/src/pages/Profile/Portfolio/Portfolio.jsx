...
import PortfolioFromExcel from "../../../components/blocks/PortfolioFromExcel/PortfolioFromExcel.jsx";
                ...
                <Tile>
                    <FlexLayout className={globalStyles.padding_0}>
                        {isAdd ? (
                            ...
                            <>
                                <Button isShort onClick={() => setIsAdd(true)}>
                                    + Добавить
                                </Button>
                            </>
                        )}
                        {isAddFile ? (
                            <div className={cn(globalStyles.between, globalStyles.flex)}>
                                <h3 className={cn(globalStyles.start, globalStyles.margin_block_02)}>Импорт из Excel файла</h3>
                                <CloseButton onClick={() => setIsAddFile(false)} />
                            </div>
                        ) : (
                            <Button isShort onClick={() => setIsAddFile(true)}>
                                Загрузить из файла
                            </Button>
                        )}
                    </FlexLayout>
                ...
                >
                    <FlexLayout type={LAYOUT_TYPES.VERTICAL} className={styles.main_tile}>
                        ...
                        {userState.portfolio.length > 0 ? (
                            ...
                            <div>Вы ещё не добавили портфолио</div>
                        )}
                    </FlexLayout>
                </Tile>
            </FlexLayout>
